...
| F-17 | ~~`validate_attribute_name_sizes` only checks top-level attribute names — nested map keys not validated~~ The validator now walks `M` and `L` values and applies the same byte limit to every map key on the way down, so PutItem, BatchWriteItem, TransactWriteItems Put, and ImportTable all reject an oversized nested name with the top-level message. The nested wording is not separately measured against the service; the naming rules state the limit without a top-level qualifier | ~~`core/validation/mod.rs`~~ | ~~Low~~ | P30 |
