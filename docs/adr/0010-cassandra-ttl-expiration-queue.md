...
If the item really is expired, the worker moves the queue row through three states, and the states exist so that a crash at any point can be resumed rather than guessed at. It first claims the row, writing into it the exact image of the item it intends to delete, a fixed delete timestamp, and — if the table has a stream — the event ID and sequence number it will use, decided now rather than at write time. It then takes the claim on the item's row, and with that held it deletes the item's secondary-index rows and writes the `REMOVE` stream record. Because the stream identity was decided up front, retrying this step overwrites the same stream row instead of publishing a second event. It marks the queue row as having applied those effects, and only then deletes the item itself, conditional on both the claim still being its own and the item still being byte-for-byte what it recorded. Finally it deletes the queue row.

The ordering matters in one direction in particular. Once effects have been applied, the item's index rows are already gone and the `REMOVE` has already been published, so that work has to be finished rather than abandoned — otherwise a live item is left with a missing index. Everything before that point can be safely walked away from, because nothing outside the queue has changed yet. Retiring a generation follows the same rule: it drops queue rows that were only pending, releases and abandons rows that were merely claimed, and pushes rows that had already applied their effects through to completion.
