...
| TTL-enabled write cost | Unaffected by TTL | An extra catalog read (overlapped with the index read) and a claim LWT, with the base write becoming a logged batch. The release is folded into the batch, with an exact conditional release performed off the request path. Non-TTL tables keep the existing fast paths. |
