...
| TTL deletion | Background process, items deleted within 48 hours of expiry | Background worker with an indexed sweep. On Cassandra the interval and batch size are fixed (60s, 100 items per table per cycle) and are not configurable. |
...
| TTL attribute value validation | Non-conforming values are ignored | Same intent: only a positive integral `N` is queued. Missing, non-numeric, fractional, zero, and negative values are ignored. |
| `UpdateTimeToLive` response timing | Returns immediately; the change takes effect asynchronously (up to about an hour) | On Cassandra the call awaits a synchronous full-table backfill of the expiration queue, so it can be slow on a large table and can exceed a client timeout. The backfill has no durable cursor; a failure restarts it on the next worker cycle. |
...
| Cassandra TTL expiration throughput | Managed by the service | Bounded by design: one exclusive sweep lease per table and one bounded batch per 60-second cycle, so about 100 items per table per minute cluster-wide regardless of fleet size. A table expiring faster than that accumulates backlog. |
