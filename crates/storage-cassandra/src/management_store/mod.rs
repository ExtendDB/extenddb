...
//! `ManagementStore` implementation for `CassandraCatalogStore`.
//!
...
impl extenddb_storage::management_store::ManagementStore for CassandraCatalogStore {
    // ── Accounts ───────────────────────────────────────────────────
    ...
    fn default_account_id(&self) -> BoxFuture<'_, OpResult<Option<String>>> {
        Box::pin(async move { self.get_default_account_id_impl().await })
    }
