...
/// Number of fixed shards per stream.
...
impl StreamEngine for SqliteEngine {
        ...
        Box::pin(async move {
            ...
            sqlx::query(
                "INSERT INTO stream_records \
                  (sequence_number, shard_id, table_id, event_name, record_data) \
                 VALUES (?, ?, ?, ?, ?)",
            )
    ...
    fn get_stream_records(
        ...
        Box::pin(async move {
            let rows: Vec<(String,)> = if let Some(after) = after_sequence {
                sqlx::query_as(
    ...
    fn describe_stream(
        ...
        Box::pin(async move {
            ...
            let row: Option<(String, String, Option<String>, String, String)> = sqlx::query_as(
                "SELECT key_schema, attribute_definitions, stream_specification, \
            ...
            let (ks_str, _ad_str, stream_spec_str, table_status, table_id) =
                row.ok_or_else(|| {
            ...
            let key_schema = serde_json::from_str(&ks_str)
                .map_err(|e| StorageError::Internal(e.to_string()))?;

            let stream_view_type = stream_spec_str
                .as_deref()
            ...
            let shard_rows: Vec<(String, Option<String>, String, Option<String>)> =
                if let Some(ref start) = exclusive_start_shard_id {
                    sqlx::query_as(
                        "SELECT shard_id, parent_shard_id, starting_sequence_number, \
    ...
    fn list_streams(
        ...
        Box::pin(async move {
            let rows: Vec<(String, String, String)> = match (
                    ...
                    sqlx::query_as(
                        "SELECT table_name, table_arn, stream_label FROM tables \
                         WHERE account_id = ? AND stream_label IS NOT NULL \
                           AND table_name = ? AND stream_label > ? \
                ...
                .map_err(|e| StorageError::Internal(e.to_string()))?,
                (None, Some(start_arn)) => {
                    // SQLite doesn't support tuple comparison; expand it.
                    let (start_table, start_label) = parse_stream_arn(start_arn)?;
                    sqlx::query_as(
                        "SELECT table_name, table_arn, stream_label FROM tables \
                         WHERE account_id = ? AND stream_label IS NOT NULL \
                           AND (table_name > ? OR (table_name = ? AND stream_label > ?)) \
    ...
    fn next_sequence_number(&self, _shard_id: &str) -> BoxFuture<'_, Result<String, StorageError>> {
        Box::pin(async move {
            let (seq_val,): (i64,) = sqlx::query_as(
                "UPDATE seq_counters SET value = value + 1 WHERE name = 'stream' RETURNING value",
            )
            .fetch_one(&self.pool)
            .await
            .map_err(|e| StorageError::Internal(e.to_string()))?;
    ...
    fn validate_shard(
        ...
        Box::pin(async move {
            ...
            let exists: Option<(i32,)> = sqlx::query_as(
                "SELECT 1 FROM stream_shards WHERE shard_id = ? AND table_id = ?",
            )
            .bind(&shard_id)
            .bind(&table_id)
            .fetch_optional(&self.pool)
