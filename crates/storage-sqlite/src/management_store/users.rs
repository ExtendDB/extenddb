...
//! User management operations for `SqliteCatalogStore`.
...
impl SqliteCatalogStore {
    pub(crate) async fn create_user_impl(
        ...
        let policy_doc_str =
            serde_json::to_string(&self_service_policy).unwrap_or_else(|_| "{}".to_owned());
    ...
    pub(crate) async fn delete_user_impl(
        &self,
        account_id: &str,
        user_name: &str,
    ) -> OpResult<()> {
        let result =
            sqlx::query("DELETE FROM iam_users WHERE account_id = ? AND user_name = ?")
                .bind(account_id)
                .bind(user_name)
                .execute(self.pool())
                .await;
    ...
    pub(crate) async fn list_users_impl(
        ...
        rows.into_iter()
            .map(|(aid, un, arn, pw, ts)| {
                let created_at =
                    crate::sqlite_util::parse_timestamp(&ts).map_err(|e| {
                        tracing::error!("list_users parse_timestamp: {e}");
                        OpError::Internal("Database error".to_owned())
                    })?;
                Ok((aid, un, arn, pw.is_some(), created_at))
            })
            .collect()
    }
        ...
        for (key, value) in tags {
            let result = sqlx::query(
                "INSERT INTO iam_user_tags (account_id, user_name, tag_key, tag_value) \
                 VALUES (?, ?, ?, ?) \
                 ON CONFLICT (account_id, user_name, tag_key) \
    ...
    pub(crate) async fn untag_user_impl(
        ...
        for key in tag_keys {
            sqlx::query(
                "DELETE FROM iam_user_tags \
                 WHERE account_id = ? AND user_name = ? AND tag_key = ?",
            )
            .bind(account_id)
            .bind(user_name)
            .bind(key)
