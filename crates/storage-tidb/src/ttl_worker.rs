...
//! TTL cleanup background worker for TiDB.
...
pub(crate) async fn ttl_cleanup_worker(storage: Arc<TidbEngine>, metrics: Arc<MetricsCollector>) {
...
async fn sweep_expired_items(storage: &TidbEngine, metrics: &MetricsCollector, region: &Arc<str>) {
    let ttl_identity = UserIdentity {
        identity_type: "Service".to_owned(),
        principal_id: "dynamodb.amazonaws.com".to_owned(),
    };

    let tables = match storage.streaming_ttl_tables_ready().await {
        Ok(t) => t,
        Err(e) => {
